...
static int rtk_usb3phy_probe(struct platform_device *pdev)
{
    ...
    if (dev->of_node) {
        ...
        if (realtek_rev() == RTK1195_REV_A) {
            ret = of_property_read_u16_array(dev->of_node, "phy_data_revA", phy_data->data, phy_data->size);
        } else if (realtek_rev() == RTK1195_REV_B) {
            ret = of_property_read_u16_array(dev->of_node, "phy_data_revB", phy_data->data, phy_data->size);
        } else if (realtek_rev() == RTK1195_REV_C) {
            ret = of_property_read_u16_array(dev->of_node, "phy_data_revC", phy_data->data, phy_data->size);
        } else if (realtek_rev() == RTK1195_REV_D) {
            ret = of_property_read_u16_array(dev->of_node, "phy_data_revD", phy_data->data, phy_data->size);
        } else {
            dev_err(dev, "No phy_data in realtek_rev=%d\n", realtek_rev());
            ret = -ENODEV;
        }
#endif
        if (ret)
